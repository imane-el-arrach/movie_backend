...
def get_db():
    db = SessionLocal()
    try:
        yield db # Gestion automatique d'ouverture et de fermeture
    finally:
        db.close()
...
@app.get(
    ...
    # Permet à fastapi de valider et de formater automatiquement la repose Json avec un schemas pythandic
    response_model=schemas.MovieDetailed, # (MovieDetailed se regroupe dans le schemas.py)
    tags=["films"], # regroupement des endpoints dans swagger
)
