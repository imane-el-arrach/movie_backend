...
from .schemas import (
    MovieSimple, MovieDetailed,
    RatingSimple, TagSimple, LinkSimple,
    AnalyticsResponse
)
from .film_config import MovieConfig
...
class MovieClient:
    ...
    ) -> Union[List[object], List[dict], pd.DataFrame]:
        if output_format == "pydantic":
            return [model(**item) for item in data]
        elif output_format == "dict":
            return [dict(item) if hasattr(item, "__dict__") else item for item in data]
